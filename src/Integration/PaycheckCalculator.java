 ...
 * 
 * The PaycheckCalculator is a 3-part program that can be used by payroll clerks to calculate wages
 * and paycheck stubs for employees.
 * 
 * There are 8 primitive data types in Java:
 * 
 * byte: The byte data type is an 8-bit signed two's complement integer. has a minimum value of -128
 * and a maximum value of 127 (inclusive). The byte data type can be useful for saving memory in
 * large arrays, where the memory savings actually matters.
 * 
 * short: The short data type is a 16-bit signed two's complement integer. has a minimum value of
 ...
 * values is beyond the scope of this discussion, This data type should never be used for precise
 * values, such as currency. For that, you will need to use the java.math.BigDecimal class instead. 
 * 
 * double: Its range of values is beyond the scope of this discussion, but is specified in
 * the Floating-Point Types, Formats, and Values section of the Java Language Specification. For
 * decimal values, this data type is generally the default choice. this data type should never be
 * used for precise values, such as currency.
 * 
 * boolean: The boolean data type has only two possible values: true and false. char: The char data
...
/**
 * This is a driver class that holds the main concepts of what I've learned through out this course.
 * 
 * @author
 * Justin Rubio
 *
...
public class PaycheckCalculator {
  public static void main(String[] args) throws InputMismatchException {
    ...
    try { // try- catch will prevent program from crashing from bad input.
      ...
      do {
        if (hourlyWage < 10 || hourlyWage > 500) {
          System.out.println("Be realistic. Wages are between 10 and 500 an hour. Try again.");
      ...
      while (weeklySales == 0) {
        ...
        if (weeklySales == 0) {
          ...
          if (weeklySales == 0) {
            ...
            if (weeklySales == 0) {
              System.out.println(
                  "Yes, I seriously nested 3 if-statements inside of a while loop. Sell something or you are fired.");
              weeklySales = scanner.nextDouble();
              if (weeklySales == 0) {
                ...
                // sold something.
              } else { // else statement is here to taunt the user after abusing them into making
                       // sell something
      ...
      double total;
      total = TotalsUsingMethods.totalIncomeFromHours(totalHours, hourlyWage) + COMMISSION;
      ...
      String yesOrNo = usersAnswer.equals("y") ? "Great!"
          : "Regardless of your answer, this is my project, you will stay and suffer.";
      ...
      int forcedGamble = random.nextInt(5) + 0;// Chooses a random intn between 0 and 5
      // SpotBug problem has been researched and analyzed. Must set to a constant random, to be able
      // to use one alone.
      ...
      switch (forcedGamble) {
        ...
        case 5:
          ...
          break;
        default:
          System.out.println("Somehow, you didn't gamble... very responsible.");
      ...
      System.out
          .printf(coWorkerName + "'s Stats for the Week: \n" + coWorker.averageEmployeeStats());
      ...
      int smallestValue = spreadSheet[0];
      ...
      for (int index = 0; index < spreadSheet.length; index++) {
        sumOfTheSpreadSheet = sumOfTheSpreadSheet + spreadSheet[index];
        if (smallestValue > spreadSheet[index]) {
          // properly locates smallest index value
          smallestValue = spreadSheet[index];
      ...
      System.out.println("Sum of the array = " + sumOfTheSpreadSheet
          + "\nIndex of the smallest Value = " + locationOfSmallestValue);
      ...
      int[][] comparedSpreadSheets =
          {{(int) hourlyWage, (int) totalHours, (int) weeklySales}, {20, 40, 400}};
      int smallestValue2 = comparedSpreadSheets[0][0];
      int locationOfSmallestValue2 = 0;
      System.out.println("Yours:\t" + coWorkerName + "'s: ");
      for (int row = 0; row < comparedSpreadSheets.length; row++) {
        for (int column = 0; column < comparedSpreadSheets[row].length; column++) {
          System.out.println(comparedSpreadSheets[row][column]);
          // System.out.print(comparedSpreadSheets[row][column] + "\t");
          if (smallestValue2 > comparedSpreadSheets[row][column]) {
            // properly locates smallest index value
            smallestValue2 = comparedSpreadSheets[row][column];
      ...
      System.out.println("Smallest Value in the 2d array: " + smallestValue2);

      System.out.println(
          "\n" + coWorkerName + " is proud of his useless array that compares your data with his.");

      System.out.println("Wait a second... \n" + coWorkerName
          + " looks like he's going crazy with his programming!");
      ...
      System.out.println("You can't do that. Follow directions nextime. Run the program again.");
    ...
    // Leading on from inheritance, Polymorphism is the ability for objects to be able to take on
    // many forms.
